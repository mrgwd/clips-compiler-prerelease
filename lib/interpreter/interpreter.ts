...
function evaluateOperator(operator: string, args: Node[], env: Environment): Value {
  ...
  switch (operator) {
    ...
    case "/":
      if (evaluatedArgs.length === 0) throw new Error("Division requires operands")
      if (typeof evaluatedArgs[0] !== "number") {
...
function evaluateFunction(funcName: string, args: Node[], env: Environment): Value {
  switch (funcName) {
    case "bind":
      if (args.length !== 2) {
    ...
    case "loop-for-count":
      if (args.length < 2) {
        throw new Error("loop-for-count requires at least loop parameters and an action")
      }

      // Extract loop parameters
      if (args[0].type !== "Expression") {
        throw new Error("First argument to loop-for-count must be a parameter list")
      }

      const loopParams = args[0].children
      if (!loopParams || loopParams.length !== 3) {
        throw new Error("Loop parameters must include variable, start, and end values")
      }
    ...
    case "if":
      if (args.length < 3) {
        throw new Error("if requires condition, then clause, and optionally an else clause")
      }
